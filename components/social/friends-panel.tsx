...
import { Search, UserPlus, Check, X, MessageCircle, Clock, Loader2, Users } from "lucide-react"
import { socialFetcher, type SocialUser } from "@/components/social/types"

type Status = "none" | "friends" | "incoming" | "outgoing"
type PersonResult = SocialUser & { status: Status }
...
// A single person row with the correct action for my relationship to them.
function PersonRow({
  u,
  onSend,
  onOpenChat,
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-900 p-3">
      ...
      {u.status === "friends" ? (
        ...
        <button onClick={() => onSend(u.id)} className="flex items-center gap-1 rounded-full bg-emerald-500 px-3 py-1.5 text-xs font-bold text-slate-950">
...
export function FriendsPanel({ onOpenChat }: { onOpenChat: (u: SocialUser) => void }) {
  ...
  const { data: allData, mutate: mutateAll } = useSWR("/api/friends/all", socialFetcher, { refreshInterval: 20000 })
  const [q, setQ] = useState("")

  const everyone: PersonResult[] = allData?.results ?? []

  // Filter the full list locally so people show up instantly without searching.
  ...
  async function sendRequest(id: number) {
    // Optimistically flip to "outgoing" in the everyone list.
    mutateAll(
      (cur: { results?: PersonResult[] } | undefined) =>
        cur ? { ...cur, results: (cur.results ?? []).map((u) => (u.id === id ? { ...u, status: "outgoing" } : u)) } : cur,
      { revalidate: false },
    )
    ...
    <div className="mx-auto w-full max-w-lg space-y-6 px-4 py-4">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Shakisha umuntu (izina, username canke aho aba)"
      ...
      {/* Everyone who registered — shown by default, no searching required. */}
      <div className="space-y-2">
        <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-500">
          <Users className="h-3.5 w-3.5" /> Abantu bose biyandikishije ({everyone.length})
        </p>
        {!allData ? (
          <div className="flex items-center gap-2 text-sm text-slate-500"><Loader2 className="h-4 w-4 animate-spin" /> Turiko turazana abantu…</div>
        ) : filtered.length === 0 ? (
          <p className="text-sm text-slate-500">Nta muntu aboneka.</p>
        ) : (
          filtered.map((u) => <PersonRow key={u.id} u={u} onSend={sendRequest} onOpenChat={onOpenChat} />)
        )}
      </div>

      {outgoing.length > 0 ? <p className="text-xs text-slate-600">Wasavye {outgoing.length} bantu, barindiriye kwemera.</p> : null}
    </div>
  )
}
